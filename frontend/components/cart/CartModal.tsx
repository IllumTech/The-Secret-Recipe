...
export default function CartModal({ isOpen, onClose }: CartModalProps) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[9999]">
      ...
      {/* Modal Panel */}
      <div 
        className={`absolute right-0 top-0 h-screen w-full sm:w-[480px] bg-gradient-to-b from-purple-50 to-pink-50 shadow-2xl overflow-hidden transition-transform duration-300 ease-out ${
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6" style={{ maxHeight: 'calc(100vh - 280px)' }}>
            {items.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-12">
                ...
                <p className="text-gray-600 text-lg sm:text-xl font-semibold">Tu carrito está vacío</p>
          ...
          {/* Footer - Fixed */}
          {items.length > 0 && (
            <div className="flex-shrink-0 border-t border-purple-200 bg-white/90 backdrop-blur-sm p-4 sm:p-6 space-y-3 sm:space-y-4 shadow-lg">
              ...
              <div className="space-y-2 sm:space-y-3">
                <Link
                  href="/checkout"
                  onClick={onClose}
                  className="block w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white py-3 sm:py-4 rounded-lg font-semibold hover:from-purple-700 hover:to-pink-700 transition-all duration-300 shadow-lg hover:shadow-xl text-center"
                >
                  Proceder al Pago 💳
                </Link>
                ...
                <Link
                  href="/carrito"
                  onClick={onClose}
                  className="block w-full bg-white border-2 border-purple-300 text-purple-700 py-2 sm:py-3 rounded-lg font-medium hover:bg-purple-50 transition-all text-center"
                >
